...
#[derive(Debug, Clone, Eq, PartialEq, Serialize)]
pub struct Players(HashMap<PID, Role>);
...
impl Game {
    ...
    pub fn handle_vote(&mut self, voter: PID, ballot: Option<Choice>) -> Result<Option<Election>> {
        ...
        self.event_output.send(Event::Vote { voter, ballot })?;
        return Ok(result);
    }
    ...
    pub fn handle_reveal(&self, actor: PID) -> Result<()> {
        ...
        if blocks.contains(&actor) {
            self.event_output.send(Event::Block { blocked: actor });
            return Ok(());
        }
        let role = self.players.role(actor)?;
        self.event_output.send(Event::Reveal { actor, role })?;
        Ok(())
    }

    pub fn handle_target(&mut self, actor: PID, target: Choice) -> Result<bool> {
        self.players.check(actor)?;
        let Phase::Night { targets, mark, .. } = &mut self.phase else {
        ...
        self.event_output.send(Event::Target { actor, target })?;
    ...
    pub fn dusk_vote(&mut self, voter: PID, ballot: Option<Choice>) -> Result<bool> {
        ...
        match ballot {
            Some(Choice::Player(votee)) if voters.contains(&votee) => {
                self.players.check(votee)?;
                self.event_output.send(Event::Revenge {
                    avenger: voter,
                    votee,
                })?;
    ...
    pub fn elect(&mut self, election: Election, voters: HashSet<PID>) {
        ...
        self.event_output.send(Event::Election {
            election,
            voters: voters.clone(),
        });
        match election {
            Election::Hammer(elected, hammer) => {
                ...
                if role.kind() == RoleKind::IDIOT {
                    // dusk!
                    self.event_output.send(Event::Reveal {
                        actor: elected,
                        role,
                    });
                    self.phase = Phase::new_dusk(day, elected, hammer, voters.clone());
                    self.event_output.send(Event::Dusk {
                        avenger: elected,
                        voters: voters.clone(),
                    });
                    return;
    ...
    pub fn check_dawn(&self) -> bool {
        let Phase::Night { targets, mark, .. } = &self.phase else {
...
fn try_election(game: Arc<Mutex<Game>>, election: Option<Election>, force: bool) {
    ...
    let result = game.check_election(election, force);
    if let Some((election, voters)) = result {
